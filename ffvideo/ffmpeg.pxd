...
cdef extern from "libavutil/avutil.h":
	...
	cdef enum DictFlags:
		...
		AV_DICT_DONT_STRDUP_KEY = 4,	#< Take ownership of a key that's been allocated with av_malloc() and children.
		AV_DICT_DONT_STRDUP_VAL = 8,	#< Take ownership of a value that's been allocated with av_malloc() and chilren.
...
cdef extern from "libavformat/avformat.h":
	...
	int avformat_open_input(AVFormatContext **ic_ptr, char *filename, AVInputFormat *fmt, AVDictionary **options)
...
cdef extern from "libswscale/swscale.h":
	...
	int sws_scale(SwsContext *context, const uint8_t *const srcSlice[], const int srcStride[], int srcSliceY,
			int srcSliceH, uint8_t *const dst[], const int dstStride[]) nogil
