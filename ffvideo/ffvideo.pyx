# ffvideo.pyx
#
# Copyright (C) 2015 Radek Senfeld <radek.senfeld@gmail.com>
...
cdef class VideoStream:
	...
	def __init__(self, filename, frame_size=None, frame_mode="RGB", scale_mode=BICUBIC):
		cdef int ret
		cdef int i

		if isinstance(filename, unicode) or \
			(PY_MAJOR_VERSION >= 3 and isinstance(filename, str)):
		...
		cdef AVDictionaryEntry *metadata_entry = NULL
		self.metadata = {}

		while True:
			metadata_entry = av_dict_get(self.stream.metadata, "", metadata_entry, AV_DICT_IGNORE_SUFFIX)

			if not metadata_entry:
				break

			self.metadata[metadata_entry.key] = metadata_entry.value
	...
	def __decode_next_frame(self):
		...
		while not frame_finished:
			...
			if self.packet.stream_index == self.streamno:
				with nogil:
					ret = avcodec_decode_video2(self.codec_ctx, self.frame, &frame_finished, &self.packet)
				if ret < 0:
					av_free_packet(&self.packet)
...
#		print "ts=%.3f" % av_q2d(av_mul_q(AVRational(pts-self.stream.start_time, 1), self.stream.time_base))
		...
		self.frame.pts = av_rescale_q(pts-self.stream.start_time, self.stream.time_base, AV_TIME_BASE_Q)
		self.frame.display_picture_number = <int>av_q2d(
			av_mul_q(
				av_mul_q(
					AVRational(pts - self.stream.start_time, 1),
	...
	def dump_next_frame(self):
		...
		print "f.coded_picture_number=%s, f.display_picture_number=%s" % (self.frame.coded_picture_number, self.frame.display_picture_number)
	...
	def current(self):
		...
		buflen = avpicture_get_size(self.ffmpeg_frame_mode, self.frame_width, self.frame_height)
		data = bytearray(buflen)
		cdef char [:] data_view = data

		with nogil:
			avpicture_fill(<AVPicture *>scaled_frame, <uint8_t *>&data_view[0], self.ffmpeg_frame_mode, self.frame_width, self.frame_height)
			...
			sws_scale(img_convert_ctx,
				<const uint8_t * const*>self.frame.data, self.frame.linesize, 0, self.height,
	...
	def get_frame_no(self, frameno):
		cdef int64_t gpts = av_rescale(frameno, self.stream.r_frame_rate.den*AV_TIME_BASE, self.stream.r_frame_rate.num)
	...
	def get_frame_at_pts(self, int64_t pts):
		...
		stream_pts = av_rescale_q(pts, AV_TIME_BASE_Q, self.stream.time_base) + self.stream.start_time
		ret = av_seek_frame(self.format_ctx, self.streamno, stream_pts, AVSEEK_FLAG_BACKWARD)
	...
	def __iter__(self):
		# rewind
		ret = av_seek_frame(self.format_ctx, self.streamno, self.stream.start_time, AVSEEK_FLAG_BACKWARD)
...
cdef class VideoFrame:
	...
	def image(self):
		...
		return Image.frombuffer(self.mode, self.size, bytes(self.data), "raw", self.mode, 0, 1)
